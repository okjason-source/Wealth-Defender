...
export class AudioManager {
  ...
  private masterVolume: number = 0.35; // Reduced for game sounds
  ...
  private backgroundMusicVolume: number = 0.25; // Background music level
  ...
  /**
   * Load background music from public asset
   */
  private async loadBackgroundMusic(): Promise<void> {
    if (!this.audioContext) return;

    try {
      const response = await fetch(`${import.meta.env.BASE_URL}Diamonds_in_8_bits.m4a`);
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}`);
      }
      const arrayBuffer = await response.arrayBuffer();
      this.backgroundMusicBuffer = await this.audioContext.decodeAudioData(arrayBuffer);
      // Start playing background music once loaded
  ...
  public startBackgroundMusic(): void {
    if (!this.audioContext || !this.backgroundMusicBuffer || !this.enabled) return;
    ...
    gainNode.gain.value = this.backgroundMusicVolume * this.masterVolume;
    ...
    source.onended = () => {
      if (this.isBackgroundMusicPlaying) {
        this.startBackgroundMusic(); // Restart if still supposed to be playing
      }
    };
  }
  ...
  setEnabled(enabled: boolean): void {
    ...
    if (!enabled) {
      ...
      // Start background music when enabling
      if (this.backgroundMusicBuffer && !this.isBackgroundMusicPlaying) {
        this.startBackgroundMusic();
      }
    }
  }
